'''
@Project ：H-PINN 
@File    ：Klein_Gordon.py
@IDE     ：PyCharm 
@Author  ：Pancheng Niu
@Date    ：2024/8/10 上午9:37 
'''
import numpy as np
import torch
import matplotlib.pyplot as plt
from scipy.interpolate import griddata
from Klein_Gordon_PINN import Klein_Gordon_PINN,Sampler
import warnings
import os
import csv
...
if __name__ == '__main__':
    ...
    results = []# Save relative error
    for depth in [3, 5, 7]:
        for widths in [30, 50, 70, 90]:
            layer = [2] + [widths] * depth + [1]
            layer_str = f'{depth}x{widths}'
            for mode in ['PINN', 'IFNN-PINN', 'A-PINN', 'H-PINN']:
                model = Klein_Gordon_PINN(layer, operator, ics_sampler, bcs_sampler, res_sampler, alpha, beta, gamma, k,mode)
                ...
                plt.subplot(1, 3, 3)
                plt.imshow(np.abs(U_star - U_pred), extent=(t.min(), t.max(), x.min(), x.max()), origin='lower',
                           cmap='jet',
                           aspect='auto')
                plt.colorbar()
                plt.xlabel('$t$')
                ...
                ax.plot(loss_ic, label='$\mathcal{L}_{0}$')
                ax.set_yscale('log')
