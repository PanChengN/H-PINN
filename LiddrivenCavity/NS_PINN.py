...
class Navier_Stokes2D:
    def __init__(self, model_type, layers, bcs_sampler, res_sampler, operator, Re):
        ...
        self.lamu = torch.tensor([1.], requires_grad=True).float().to(device)
        ...
        self.optimizer3 = torch.optim.Adam([self.lamu], lr=0.002, maximize=True)
    ...
    def AW_loss_fun(self, loss_bcs, loss_res):
        loss = self.lamr * loss_res + self.lamu * loss_bcs
        return loss
    ...
    def train(self, nIter=10000, batch_size=128):
        ...
        for it in pbar:
            ...
            self.lam_u_log.append(self.lamu.item())
            ...
            if it % 100 == 0:
                pbar.set_postfix({'Iter': self.iter,
                                  ...
                                  'Loss_b': '{0:.3e}'.format(loss_bcs.item()),
                                  'Loss_r': '{0:.3e}'.format(loss_res.item()),
                                  'lam_r': '{0:.2f}'.format(self.lamr.item()),
                                  'lam_u': '{0:.2f}'.format(self.lamu.item())
                                  })
            self.iter += 1
