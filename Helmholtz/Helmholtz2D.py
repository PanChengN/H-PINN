...
if __name__ == '__main__':
    ...
    for depth in [3, 5, 7]:
        for widths in [30, 50, 70, 90]:
            ...
            layer_str = f'{depth}x{widths}'  # Converts the network structure into a concise string format
            for mode in ['PINN', 'IFNN-PINN', 'A-PINN', 'H-PINN']:
                ...
                plt.tight_layout()
                plt.savefig(f'./pic_k_1/{mode}_{layer_str}_Exact_Predicted_Absolute_error.pdf',
                ...
                ax.plot(loss_bcs, label='$\mathcal{L}_{b}$')
                ax.set_yscale('log')
                ax.set_xlabel('iterations')
                ax.set_ylabel('Loss')
                plt.legend()
                plt.tight_layout()
                plt.savefig(f'./pic_k_1/{mode}_{layer_str}_loss.pdf', dpi=300, bbox_inches='tight')
                plt.close(fig_2)
                # plt.show()
                ...
                plt.legend()
                plt.tight_layout()
                plt.savefig(f'./pic_k_1/{mode}_{layer_str}_lam_log.pdf', dpi=300, bbox_inches='tight')
                plt.close(fig_3)
                # plt.show()
            print(
                "***************************************************************************************************************")
